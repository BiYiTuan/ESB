package eab.console.figure
{
	...
	import eab.console.other.FigureColorManager;
	import eab.console.view.NodeContextPanel;
...
//		public var nodeName:String = "No Name";
		...
		public function handleCreateLink(event:Event):void{
			...
			if(activeModel==null){
				throw new Error("no FigureEditorModel!");
			}
			this.hideContextPanel();
			new FigureCanvasAppEvent(FigureCanvasAppEvent.Node_Create_Link,
				{fileID :activeModel.fileID ,startFigure :this} ).dispatch();
		}
		...
		override public function readInformationToFigure(info:XML):void{
			...
			this._stateToken=info.@state;
			this._inToken=info.@input;
			this._outToken= info.@output;
		}
		...
		protected function getStateColor():uint{
			var stateValue : String = getParameter(_stateToken);
			
			if(stateValue != null)
				return FigureColorManager.getColor(int((new XMLDocument(stateValue)).lastChild.firstChild.nodeValue));
			else
				return 0x000000;
		}			
	}
