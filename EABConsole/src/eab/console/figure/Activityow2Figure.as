...
{
	import eab.console.view.ActivityParameterWindows;
	import eab.console.vo.ActivityAttribute;
	...
	public class Activityow2Figure extends ow2Figure
	{
		...
		{
			super();
			this.attribute = new ActivityAttribute();
			drawid=103;
			standardwidth=110;
			standardheight=38;
			radius=8;
			...
			BindingUtils.bindProperty(this,"serviceName", this.attribute,"serviceName");
		}
		
		protected function addContextMenu():void{
			 var cmiAdd:ContextMenuItem = new ContextMenuItem("查看参数信息", true);
             cmiAdd.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT, showParameters);
  
             contextMenu = new ContextMenu();
             contextMenu.hideBuiltInItems();
             contextMenu.customItems = [cmiAdd];
		}
		
		public function showParameters(event : ContextMenuEvent):void{
			var popup:ActivityParameterWindows = ActivityParameterWindows(
				PopUpManager.createPopUp(DisplayObject(Application.application), ActivityParameterWindows, true));
			PopUpManager.centerPopUp(popup);
			popup.inPara = getParameter(_inToken);
			popup.outPara = getParameter(_outToken);
		}
		...
		override public function drawpicture():void{
			...
			sprt.graphics.lineStyle(this.defaultLineThickness*this.multiple,getStateColor(),1);
